...
(function(){function D(a,b,c){var d=null;if(q(a)){d=E();var g=F(b);e[d]={};e[d].options=g;e[d].domElementId=c;t(c)?(a=u.getElementById(c),n(a)&&(e[d].cachedObject=a.innerHTML,e[d].originalObject=a.innerHTML)):(e[d].cachedObject=JSON.stringify(a),e[d].originalObject=a);e[d].timer=setInterval(function(){var k=new Date,l=d,m=t(e[l].domElementId),h=null;m&&(h=u.getElementById(e[l].domElementId),n(h)&&(e[l].originalObject=h.innerHTML));var r=e[l].cachedObject,f=e[l].originalObject,w=m?f:JSON.stringify(f);
r!==w&&(f=e[l].options,f.reset?m?h.innerHTML=e[l].cachedObject:e[l].originalObject=x(r).result:e[l].cachedObject=w,m?z(f.onChange,r,w):(m=x(r).result,h=x(w).result,z(f.onChange,m,h),v(f.onPropertyChange)&&!G(m)&&H(m,h,f)),f.cancelOnChange&&y(l));I(g.expires)&&k>g.expires&&y(d)},g.observeTimeout)}return d}function H(a,b,c){for(var d in a)if(a.hasOwnProperty(d)){var g=a[d],k=null;b.hasOwnProperty(d)&&(k=b[d]);q(g)&&q(k)?H(g,k,c):JSON.stringify(g)!==JSON.stringify(k)&&z(c.onPropertyChange,d,g,k)}}function y(a){clearTimeout(e[a].timer);
